...
def train_func(N,
    ...
    def value_and_grad(params, sim, grad_ops, circs, beta, weights, gs, rev_mat):
        ...
        for init_index in range(len(grad_ops)):
            ...
            if circpy=="s":
                ...
                cost.append([f1, beta * f2, beta * f3])
                cost_gn.append([g1, beta * g2, beta * g3])
    ...
    for layer in range(1, num_layer + 1):
        ...
        if layer == 1:
            ...
            init_params *= 1 if circpy=="s" else np.pi
        
        else:
            init_params = joblib.load(
                'data/hardware_sym_{circpy}_Heis_params_run={}_N={}_layer={}_J={}'
        ...
        joblib.dump(
            loss_hist,
            'data/hardware_sym_{circpy}_Heis_expt_run={}_N={}_layer={}_J={}'.
        ...
        joblib.dump(
            fids_hist,
            'data/hardware_sym_{circpy}_Heis_fids_run={}_N={}_layer={}_J={}'.
            format(run, N, layer, J))

if __name__ == '__main__':
    prefix = 'p_'
    circpy = input("Enter which file")

    N = 8
    num_layer = 22 if circpy=="s" else 14
    J = 1
    sz = 0
    runs = 1
    targ_val = 0 if circpy=="s" else 2
    beta = 10 if circpy=="s" else 2
    ...
    stot = s_tot_op(N)
    pen_1 = stot ** (1 if circpy=="s" else 2)
    pen_1 = Hamiltonian(pen_1).sparse(N)
    pen_1.get_cpp_obj()
    
    if circpy != "s":
        pen_2 = -2 * targ_val * stot
        pen_2.compress()
        pen_2 = Hamiltonian(pen_2).sparse(N)
        pen_2.get_cpp_obj()
    
    ops = [ham, pen_1] if circpy=="s" else [ham, pen_1, pen_2]
    eigenstates = get_eigstates(N, sz)
    target_state = [eigenstates[1], eigenstates[2]] if circpy=="s" else [eigenstates[0], eigenstates[3]]
